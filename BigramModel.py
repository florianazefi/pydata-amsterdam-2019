"""
File name: BigramModel.py
Author: Floriana Zefi
email: florianagjzefi@gmail.com
Python Version: 3
"""
import time
import pandas as pd
...
class BigramModel(object):
    ...
    def fit(self, sequences, seq_ids):
        ...
        def get_cond_bigram_proba(i, j):
            ...
            if not (i, j) in bigram_counts:
                return 1.0 / single_counts[i] / self.alpha
            return float(bigram_counts[(i, j)]) / single_counts[i]
    ...
    def plot_proba_matrix(self, log=True):
        """
        ...
        Args:
         ...
         Return: 
             2D plot
        """
        plt.imshow(self.log_probas if log else self.probas, origin="lower")

        # Set axis labels to the actual state indices
        n = self.n_states
        n_labels = n
        step = int(n / (n_labels - 1))
        positions = np.arange(0, n + 1, step)
        labels = list(self.encoder.classes_[::step]) + ["new"]
